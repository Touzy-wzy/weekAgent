...
class LogListFilesTool(Tool):
    ...
    def run(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        print(f"📂 [log_list_files] glob={glob_pattern or _lm_config.LOG_MONITOR_GLOB}")

        try:
            text = _lm_config.list_log_files_as_text(glob_pattern=glob_pattern)
            files = _lm_config.resolve_log_files(glob_pattern=glob_pattern)
